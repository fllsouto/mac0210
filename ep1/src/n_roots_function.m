...
function [x_mid] = bisection(new_sign, x, step, choice)
  ...
  for it = 1:1:3
    ...
    [ev_f_x_mid, ev_f_x_mid_prime] = evaluate_by_choice(choice, x_mid);
...
function [root] = newton(f, f_prime, x_k, choice, tol)
  x_k_1 = x_k - (f / f_prime);
  root = x_k_1;
  [ev_f_x_k_1, ev_f_x_k_1_prime] = evaluate_by_choice(choice, x_k_1);
...
function [] = intervals(choice, interval, n_inter, tol)
  ...
  printf("Choise: %d\n", choice);
  printf("Interval: [a:step:b] -- [ %d : %f : %d ]\n", interval(1), step, interval(2));
  ...
  [ev_f_x, ev_f_x_prime] = evaluate_by_choice(choice, interval(1));
  if(ev_f_x > 0)
    sign = 1;
    new_sign = 1;
  elseif(ev_f_x < 0)
  ...
  for x = range
    ...
    [ev_f_x, ev_f_x_prime] = evaluate_by_choice(choice, x);
  ...
  [ev_f_x, ev_f_x_prime] = evaluate_by_choice(choice, interval(2));
  if(ev_f_x > 0)
    new_sign = 1;
  elseif(ev_f_x < 0)
    new_sign = -1;
...
function [ev_f_x, ev_f_x_prime] = evaluate_by_choice(choice, x)
  if(choice == 1)
    ev_f_x = function1(x);
